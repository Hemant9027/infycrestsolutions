...
const bizTechImg = (num: number) => `/Business-Tech/${num}.jpg`;
  ...
  return (
    <article data-cursor="view" className="group flex shrink-0 flex-col">
      ...
      <div className="flex items-baseline justify-between gap-4 pt-5">
        <h3 className="font-display text-3xl font-light tracking-[-0.01em] text-[#171a1c]">
          {project.title}
        </h3>
      ...
      <p className="mt-1.5 text-[11px] font-medium uppercase tracking-[0.2em] text-[#171a1c]/55">
...
export function Work({ customer }: { customer: NewCustomer }) {
  ...
  useEffect(() => {
    ...
    // Small timeout to ensure images are painted
        ...
        <div className="grid gap-10 lg:grid-cols-12 lg:items-end">
          ...
          <Reveal delay={0.12} className="lg:col-span-5">
            <p className="max-w-md text-lg leading-relaxed text-white/60 lg:ml-auto">
              Four phases, six-ish weeks, zero mystery. You always know what's
              happening, what's next, and what we need from you.
            </p>
          </Reveal>
        </div>
      ...
      <div className="relative mx-auto max-w-[1600px]">
        <div className="grid gap-16 lg:grid-cols-12 lg:gap-12">
          <div className="lg:col-span-5">
            ...
            <Reveal delay={0.14}>
              <p className="mt-8 max-w-md text-lg leading-relaxed text-white/60">
                Tell us where you're headed — we'll reply within two business
                days with honest next steps, even if that's pointing you
                somewhere else.
              </p>
            </Reveal>
            <Reveal delay={0.2}>
                ...
                {status === "success" ? (
                  ...
                  >
                    ...
                    <p className="mt-5 max-w-md leading-relaxed text-white/60">
                      ...
                      every brief personally and we'll get back to you within
                      two business days.
                    </p>
                    <button
                      type="button"
                      ...
                      <div className="flex flex-wrap gap-2.5">
                        {BUDGET_OPTIONS.map((option) => (
                          <button
                            key={option.id}
                            type="button"
                            aria-pressed={form.budget === option.id}
                            onClick={() =>
                              update(
                                "budget",
                                form.budget === option.id ? "" : option.id,
                              )
                            }
                            className={`rounded-full border px-5 py-2.5 text-[11px] font-medium uppercase tracking-[0.16em] transition-all duration-300 ${form.budget === option.id ? "border-[#f08b54] bg-[#f08b54] text-[#171a1c]" : "border-white/20 text-white/60 hover:border-[#f08b54] hover:text-[#f08b54]"}`}
                          >
                            {option.label}
                          </button>
                        ))}
